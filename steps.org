...
Following needed for cloud shell
#+BEGIN_EXAMPLE shell
...
Cloud shell doesn't allow =flutter run -d chrome= by default so you need to setup a webserver
#+BEGIN_EXAMPLE shell
flutter build web
...
web preview defaults to 8080, change port to 8000
...
Then wire up events as a callback from the catalog input
